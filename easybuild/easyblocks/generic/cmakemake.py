...
class CMakeMake(ConfigureMake):
    ...
    def configure_step(self, srcdir=None, builddir=None):
        ...
        if hip_root:
            cuda_cc = build_option('cuda_compute_capabilities') or self.cfg['cuda_compute_capabilities']
            amdgcn_cc = build_option('amdgcn_capabilities') or self.cfg['amdgcn_capabilities']
            if not cuda_cc and not amdgcn_cc:
                raise EasyBuildError('Either list of CUDA compute capabilities, or list of AMDGCN capabilities,'
                                     'either via easyconfig parameter of configure option must be specified')
            if cuda_cc and amdgcn_cc:
                raise EasyBuildError('Both NVIDIA and AMD cannot be specified for \'CMAKE_HIP_PLATFORM\'. '
                                     'Please specify either cuda_compute_capabilities or amdgcn_capabilities, '
                                     'but not both.')

            if cuda_cc and not amdgcn_cc:
                options['CMAKE_HIP_PLATFORM'] = 'nvidia'
                options['CMAKE_HIP_ARCHITECTURES'] = '"%s"' % ';'.join([cc.replace('.', '') for cc in cuda_cc])
            if amdgcn_cc and not cuda_cc:
                options['CMAKE_HIP_PLATFORM'] = 'amd'
                options['CMAKE_HIP_ARCHITECTURES'] = '"%s"' % ';'.join([cc.replace('.', '') for cc in amdgcn_cc])
